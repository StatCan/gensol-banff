"""Basic test checks duplicate file output validation

If the same file path is specified for multiple output datasets in a single call,
...
def test_donorimp_duplicate_output_a(capfd, indata, donorstat):
    """Run user guide example 1 but specify same file path for multiple output datasets"""
    ...
    try:
        banff_call= banfftest.pytest_donorimp(
            indata=indata,
            instatus=donorstat,
            outdata="outfile.parquet",
            outstatus="outfile.parquet",
            unit_id="IDENT", 
