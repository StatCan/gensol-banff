/* SAS_EXPORT: START
   ...
   IMPORTANT: This code replaces missing CHARACTER data with a period ('.') in the exported file.  
              Otherwise, python cannot determine if a missing value is character or numeric.  
              Banff Python package may replace this with a space (' ') during processing.  
...
%macro export_dataset(ds_ref, ds_name);
    ...
    %generate_testcase_name() 
        ...
        /* prepare for Python: replace missing CHARACTER data*/
        data out_dir.&testcase_name._&ds_name.;
            set out_dir.&testcase_name._&ds_name.;
            array _a $ _character_;
            do over _a;
              if missing(_a) then _a='.';
            end;
        run;
    %end;
%mend;
...
%export_ds_outlier(
 ...
 SAS_EXPORT: END *//* Observations with valid key but with missing
values in the input data set for fields specified in the edits or MUSTMATCH statement and 
without status FTI in the input status data set should not be processed. A warning message 
should be entered in the log with a counter for the number of observations dropped. */
