...
%export_ds_outlier(
 ...
 SAS_EXPORT: END */ /* To verify that the output is the same for the same value of SEED. 

The same donor should selected in successive executions as several donors are 
the same distance from the recipient, and so the same donor will be randomly selected
from these ones because the seed is fixed. */
