# To create a permanent output status data set, a two-level name must be specified.
import pytest
...
def test_donorH01(capfd, indataCharKey, instatCharKey, expected_outstatus_01b):
    #### arrange
    ...
    try:
        #Create Banff call
        donorimputation_h01= banff.donorimp(
    ...
    assert_log_consistent(test_log=out_act, sas_log_path=sas_log_path, must_exist=True, msg="NOTE: OUTSTATUS = ")
    assert_dataset_equal(donorimputation_h01.outstatus, expected_outstatus_01b, "OUTSTATUS")
