...
IO_RETURN_CODE SP_init(
    ...
    PARM_init(&sp->spg, UPO_CARDINALITY,         (PARM_generic*) &sp->cardinality,        BPN_CARDINALITY,            IOPT_NUMERIC);
    PARM_init(&sp->spg, UPO_TIME_PER_OBS,        (PARM_generic*) &sp->time_per_obs,       BPN_TIME_PER_OBS,           IOPT_NUMERIC);
    ...
    PARM_init(&sp->spg, UPO_WEIGHTS,             (PARM_generic*) &sp->weights,            BPN_WEIGHTS,                IOPT_STRING);
    ...
    TIME_CPU_START(dsr_indata_init);
    rc_io = DSR_indata_init(sp, in_ds_indata);
    TIME_CPU_STOPDIFF(dsr_indata_init);
